...
async function isCracked(ip, port, version, usesProtocol, callback) {
    ...
    const username = `AuthTest${Math.round(Math.random() * 1000)}`;
    ...
    packetLength.writeUInt8(handshake.length);
    var buffer = Buffer.concat([packetLength, handshake]);
    ...
    const loginStart = Buffer.concat(buffers);
    packetLength = Buffer.alloc(1);
    packetLength.writeUInt8(loginStart.length);
    const response = await send(ip, port, Buffer.concat([buffer, packetLength, loginStart]), 6000);
